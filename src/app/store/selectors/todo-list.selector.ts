...
const selector = <T>(mapping: (state: TodoListPageState) => T) =>
  createSelector(selectTodoListFeature, mapping);
...
export const selectTodoListItemsState = selector(
  (state: TodoListPageState): TodoListItem[] => state.todoItems,
);
